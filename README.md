...
Install the `shadow-se` command onto your PATH once, then start the site from
**any** directory with a single word:

```bash
make install      # puts `shadow-se` on your PATH (~/.local/bin, $PREFIX/bin, ...)
shadow-se         # build + start web UI + admin + JSON API, open your browser, menu
...
Without installing, the same works from the project folder via `./start.sh`,
`./start.sh run`, `./start.sh onion`, `./stop.sh`, or `make run`/`make stop`.
`shadow-se`/`start.sh` are idempotent and won't double-start a service that's
already answering. Ports/mode are overridable:
`WEB_PORT=8080 ADMIN_PORT=8081 GATEWAY_PORT=8090 MODE=stub`.
